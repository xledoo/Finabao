    ...
    function on_register() {
        ...
        if(!submitcheck('regsubmit', 0)) {
            ...
            if(!$activation) {
                ...
                if($usernamelen < 5) {
                    showmessage('抱歉，您输入的用户名小于 5 个字符，请输入一个较长的用户名');
            ...
            /*=================================================================*/
            if(strlen($_mobile) != 11 || !preg_match("/^1[3|5|8][0-9]\d{4,8}$/", $_mobile)){
                showmessage('手机号码格式输入错误');
            }
            if(DB::result_first("SELECT `uid` FROM ".DB::table('common_member_profile')." WHERE `mobile`=$_mobile")){
                showmessage('该手机号码已经被注册，请重新输入。');
            }
            if(strlen($_signcode) != 6){
                showmessage('短信验证码错误');
            }
            if(!DB::result_first("SELECT `mobile` FROM ".DB::table('plugin_finabao_checkmobile')." WHERE `mobile`=$_mobile AND `signcode`=$_signcode")){
                showmessage('短信验证码错误');
            }
            ...
            $extra = array(
                ...
                'extrajs' => '<script type="text/javascript">'.
                    'setTimeout("window.location.href =\''.$url_forward.'\';", '.$refreshtime.');'.
